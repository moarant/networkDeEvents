...
if ( ! function_exists( 'political_donate_nf_default_value' ) ) :
function political_donate_nf_default_value( $data, $field_id ){
	global $ninja_forms_loading;

	if ( is_object( $ninja_forms_loading ) && isset( $ninja_forms_loading->data['form']['political_pp_amount'] ) ) {

		$amount_field = (int) $ninja_forms_loading->data['form']['political_pp_amount'];

		// Set the default value
		if ( $field_id == $amount_field ) {
			if ( isset( $_POST['amount']) && ! empty( $_POST['amount'] ) ) {
				$amount = (int) $_POST['amount']; // for security cast to integer
				$data['default_value'] = $amount;
			}
		}
	}
...
add_filter( 'ninja_forms_field', 'political_donate_nf_default_value', 10, 2 );
